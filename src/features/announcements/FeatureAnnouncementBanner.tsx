...
export function FeatureAnnouncementBanner({ userId }: { userId: string }) {
  ...
  return (
    <aside
      ...
      className="mx-4 mt-4 shrink-0 rounded-xl border border-green-200 bg-green-50 p-4"
    >
      <h2
        id="feature-announcement-heading"
        className="text-sm font-semibold tracking-wide text-green-900 uppercase"
      >
      ...
      <ul className="mt-2 space-y-3">
        {unread.map((announcement) => (
          <li key={announcement.id}>
            <h3 className="font-medium text-green-950">{announcement.title}</h3>
            <p className="text-sm text-green-900">{announcement.body}</p>
            {announcement.path && (
              <Link
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <button
          ...
          className="min-h-11 rounded-full border border-green-800 bg-green-800 px-4 py-2 text-sm text-white"
        >
          Fik den
        </button>
        <Link to="/nyheder" className="text-sm text-green-800 underline">
          Se alle nyheder
        </Link>
      </div>
    </aside>
