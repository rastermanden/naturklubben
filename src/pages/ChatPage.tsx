...
function ChatPage() {
  ...
  // På en telefon er lodret plads det knappeste: søgefelt og
  // notifikationsvalg er noget, man bruger sjældent, men de skubbede
  // beskederne ned i en stribe på et par linjer. De folder nu ud på
  // forlangende og står som før fra sm og op.
  ...
  return (
    <>
      {/* Fuldskærmen lægger sig over app-shellen -- header, menu og footer --
          men chatten skal stadig stå i en læsbar bredde på en stor skærm.
          Derfor maler et bundlag hele vinduet, og selve spalten står centreret
          ovenpå. */}
      {isFullscreen && (
        <div className="fixed inset-0 z-40 bg-app" aria-hidden="true" />
      )}
      <main
        className={`mx-auto flex min-h-0 w-full max-w-3xl flex-1 flex-col gap-3 overflow-hidden p-4 sm:gap-4 sm:p-6 ${
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button
              ...
              aria-label="Søgning og notifikationer"
              title="Søgning og notifikationer"
              className="flex h-11 w-11 items-center justify-center rounded-lg border border-line-strong text-ink-body sm:hidden"
            >
            ...
            <button
              ...
              aria-label={
                isFullscreen ? 'Afslut fuldskærm' : 'Vis chatten i fuldskærm'
              }
              title={isFullscreen ? 'Afslut fuldskærm' : 'Fuldskærm'}
              className="flex h-11 w-11 items-center justify-center rounded-lg border border-line-strong text-lg text-ink-body"
            >
        ...
        <div
          id="chat-tools"
          className={`${toolsOpen ? 'flex' : 'hidden'} shrink-0 flex-col gap-3 sm:flex`}
        >
              ...
              <div className="absolute z-10 mt-1 max-h-80 w-full overflow-y-auto rounded-lg border border-line bg-surface p-2 shadow-lg">
                ...
                {searchQuery.isSuccess &&
                  ...
                  searchResults.length === 0 && (
                    <p className="p-3 text-ink-subtle">
