...
| `event_attendance`                         | Medlemmernes svar på en begivenhed: `attending`, `waitlisted` eller `declined` (#222). Ventelisten står i `created_at`-rækkefølge.                                                                                                                                                                                                                | Alle medlemmer kan læse. INSERT/UPDATE/DELETE er revoked -- kun `respond_to_event` og `promote_event_waitlist` skriver, bag en advisory lock pr. begivenhed, så loftet ikke kan omgås.                                                                                                                      |
...
- Ledige pladser tilhører ventelisten: en ny tilmelding fylder først køen op, før den
  selv får en plads. Det gør, at et hævet loft eller en slettet konto aldrig efterlader
  ledige pladser med folk i kø.
...
- `event_members_without_response(p_event_id)` giver arrangøren og admins listen over
  ...
  viden -- hvem der _ikke_ har svaret, er kun arrangørens.
...
- `supabase/tests/rls/17_event_waitlist.sql` måler, at loftet håndhæves, at
  ...
  fylder de ledige pladser, og at listen over manglende svar er lukket for andre.
...
`00_platform.sql` lægger også de _nye_ Supabase-standardrettigheder på `public` (#209):
tabeller og sekvenser oprettes uden select/insert/update/delete/usage til `anon`,
`authenticated` og `service_role`, præcis som i et nyt projekt og på hver Preview
Branch. Det, appen bruger, skal derfor være givet eksplicit af en migration --
`20260911120000_explicit_api_grants.sql` gør det for alt, der fandtes før, og
`tests/rls/99_api_grants.sql` måler både, at bootstrappen er stram, og at de vigtigste
grants findes. En ny migration, der glemmer sin `grant`, fejler her og ikke først på
preview'et.
...
2. Nogen sender en besked. Klienten indsætter rækken og kalder bagefter `chat-push` med
   besked-id'et.
3. `chat-push` finder ud af, hvem der skal have besked (se nedenfor), krypterer et
   smugkig af beskeden til hver af dem og POSTer det til deres push-tjeneste.
...
### Mentions og hvor meget der sendes

Hvert medlem vælger på sin profil (`chat_notification_preference`), hvor meget chatten må
sende: `all`, `mentions` eller `none`. Valget er personligt og ligger derfor på `profiles`
og ikke på `push_subscriptions` -- det skal gælde alle medlemmets enheder, ikke den ene
installation, det blev sat fra. UI'et står ved siden af notifikationsknappen på `/chat` og
i admin-panelets indstillinger.

Filtreringen sker i `chat-push` (`recipients.ts`), ikke i klienten: en klient kan ikke
undlade at modtage en notifikation, den allerede har fået, og functionen er det eneste
sted, der kan lade være med at sende. Et medlem uden en gemt præference behandles som
`all`, så en manglende række aldrig gør chatten tavs.

En besked bærer selv de nævntes **bruger-id'er** i `messages.mentions`, ikke navnetekst:
danske navne har mellemrum ("Martin Jensen"), og et navneskift ville ellers efterlade en
død mention. Klienten løser `@Navn` op mod profil-kortet ved afsendelse og slår id'et op
igen ved visning, så navnet følger med et skift. Prisen er, at den gamle stavemåde i en
gammel besked ikke fremhæves efter et navneskift -- selve mention'en og notifikationen
rammer stadig den rigtige.

En mention sendes med sit eget tag (`naturklubben-chat-mention`) og sin egen titel ("X
nævnte dig"). Uden det egne tag ville den næste almindelige besked erstatte
mention-notifikationen i skuffen, og så var pointen med at kunne skrue ned væk igen.
...
1. Migrationen indsætter en række i `feature_announcements` med `push_status = 'pending'`.
   Det sker automatisk ved merge til `main`, sammen med resten af migrationen.
2. Det første medlem, der åbner appen bagefter, får nyheden med i `FeatureAnnouncementBanner`
...
### Hvorfor klienten sætter udsendelsen i gang

`probation-notifications` kalder sig selv fra databasen med `pg_net` og udleder functionens
URL af requestets `host`-header. Det kan en nyhed ikke: den oprettes af en migration, hvor
der ikke er noget request. Alternativerne ville være at gemme URL'en et sted i databasen
eller sætte et repo-secret -- begge dele er opsætning, nogen skal huske, og det sidste er
netop det manuelle dashboard-trin, projektet undgår.

Klientkaldet er den mindste vej udenom, og det er harmløst, fordi kalderen intet bestemmer:
payloaden er tom, teksten og modtagerne kommer fra databasen, og `claim`-RPC'en gør et
gentaget kald til ingenting. Åbner ti medlemmer appen samtidig, vinder den ene claim'en, og
de ni andre får `0` tilbage og sender ikke noget.
...
En nyhed sendes kun inden for syv dage efter `released_at`, og efter ti forsøg opgives den.
Ellers kunne en nyhed, der af en eller anden grund aldrig blev leveret, dukke op som en
notifikation uger senere -- eller en gammel række kunne vækkes af et miljø, der først lige
er kommet op. Falder nyheden ud af vinduet, står den stadig under `/nyheder`; det er kun
afbrydelsen, der udløber.

### Fravalg

`profiles.feature_notifications_enabled` (default `true`) styrer, om medlemmet vil have
nyhederne som notifikation. Valget ligger på profilen og ikke på `push_subscriptions`, af
samme grund som `chat_notification_preference`: det er personligt og skal gælde alle
medlemmets enheder. Filtreringen sker i functionen (`announcements.ts`), ikke i klienten --
en klient kan ikke undlade at modtage en notifikation, den allerede har fået. UI'et står på
`/nyheder`, ved siden af den almindelige notifikationsknap.
